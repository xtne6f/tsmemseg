...
void CID3Converter::AddPacket(const uint8_t *packet)
{
    ...
    else if (pid == m_pat.first_pmt.pmt_pid) {
        int done;
        do {
            done = extract_psi(&m_pat.first_pmt.psi, payload, payloadSize, unitStart, counter);
            if (m_pat.first_pmt.psi.version_number && m_pat.first_pmt.psi.table_id == 2) {
                AddPmt(pid, m_pat.first_pmt.psi);
            }
        }
        while (!done);
    }
